...
class ProductsService extends ProductsBaseService
{
    ...
    /**
     * @param ProductLanguage[] $productsLanguages
     * @return string
     */
    public function buildCsv(array $productsLanguages): string
    {
        $fname = tempnam('/tmp', 'catalog_csv');
...
//            }
    ...
    public function getSuggestions($objects)
    {
        foreach ($objects as $o) {
            if (get_class($o) == RelatedObjectClassificator::class) {
                ...
                $roc = $o;
                $simmilarClassificators = $this->catalogDao->loadSimmilarClassificators(
                    $roc->classificatorCode,
                    $roc->correctCode,
                    5
                );
    ...
    public function importProducts($csvFile)
    {
        ...
        $givenFields = $head;
        $importingFieldsProducts = array_diff(
            array_intersect($givenFields, Product::ALLOWED_FIELDS),
            Product::CLASSIFICATORS_GROUPS
        );
...
//                    $this->categoryDao->markDeletedProductCategories($delSkus); // netrinam vis dėl to
    ...
    {
        $productAndLanguageFields = array_merge(
            ['sku', 'language', 'categories', 'categories_del'],
            Product::ALLOWED_FIELDS,
            ProductLanguage::ALLOWED_FIELDS
        );
